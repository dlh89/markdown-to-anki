...
def get_questions_and_answers(source_markdown_path):
  ...
  for index, reference_link in enumerate(reference_links):
    stripped_reference_link = str(reference_link[1]).strip()
    source_markdown = source_markdown.replace(stripped_reference_link, f'[{stripped_reference_link}]({stripped_reference_link})')
  ...
  for index, question in enumerate(questions):
    ...
    markdown_question = markdown.markdown(question[1], extensions=['codehilite', 'fenced_code', 'tables'])
    markdown_answer = markdown.markdown(answer[0], extensions=['codehilite', 'fenced_code', 'tables'])
